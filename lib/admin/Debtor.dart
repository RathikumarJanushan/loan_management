        ...
        builder: (context, setDialogState) => AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                final userInputAmount = double.tryParse(amountController.text);
                ...
                bool isSkipPayment = paidRowIndex > numExistingPayments;
                ...
                if (isSkipPayment) {
                  ...
                  double runningLargeLoanBalance =
                      ((lastPaymentData?['largeLoanBalance'] as num?) ?? 0)
                          .toDouble();
                  ...
                  for (int i = numExistingPayments; i < paidRowIndex; i++) {
                    final autoFilledDueDate = DateTime(
                      loanDate.year,
                      loanDate.month + (i + 1),
                      loanDate.day,
                    );
                    ...
                    runningLargeLoanBalance -= monthlyPayment.toDouble();
                    totalSkippedPaymentAmount += monthlyPayment.toDouble();

                    final autoFilledPayment = {
                      'amountPaid': monthlyPayment,
                      'paymentDate': Timestamp.fromDate(selectedDate!),
                      'dueDate': Timestamp.fromDate(autoFilledDueDate),
                      'interest': interest,
                      'principal': principal,
                      'loanBalance': runningLoanBalance,
                      'largeLoanBalance': runningLargeLoanBalance,
                      'pendingInterest':
                          runningLargeLoanBalance - runningLoanBalance,
                      'isAutoFilled': true,
                    };
                  ...
                  final finalPrincipal = adjustedAmountPaid - finalInterest;
                  final finalLoanBalance = runningLoanBalance - finalPrincipal;
                  ...
                  final userPaidPayment = {
                    ...
                    'pendingInterest': finalLargeLoanBalance - finalLoanBalance,
                  };
                  ...
                  if (_payments.isEmpty) {
                    ...
                    final loanAmount = (loanData['loanAmount'] as num?) ?? 0;
                    final loanBalance = loanAmount.toDouble() - principal;
                    final largeLoanBalance =
                        totalPayment.toDouble() - userInputAmount;

                    newPaymentData['loanBalance'] = loanBalance;
                    newPaymentData['largeLoanBalance'] = largeLoanBalance;
                    ...
                    final prevLargeLoanBalance =
                        (lastPaymentData?['largeLoanBalance'] as num?) ?? 0;
                    final loanBalance = prevLoanBalance.toDouble() - principal;
                    final largeLoanBalance =
                        prevLargeLoanBalance.toDouble() - userInputAmount;
  ...
  DataTable _scheduleTable({
    ...
    final rows = List.generate(months, (i) {
      ...
      return DataRow(
        cells: [
          ...
          DataCell(
            Text(
              '${paymentData?['loanBalance'] ?? '-'}',
              style: const TextStyle(color: Colors.white),
            ),
          ),
          DataCell(
            Text(
              '${paymentData?['largeLoanBalance'] ?? '-'}',
              style: const TextStyle(color: Colors.white),
            ),
          ),
          DataCell(
    ...
    return DataTable(
      ...
      columns: const [
        ...
        DataColumn(
          label: Text('Date of Payment', style: TextStyle(color: Colors.white)),
        ),
        DataColumn(
        ...
        DataColumn(
          label: Text('LOAN BALANCE', style: TextStyle(color: Colors.white)),
        ),
        DataColumn(
          label: Text(
            'LARGE LOAN BALANCE',
  ...
  Widget _buildPaymentsChart() {
    ...
    Widget bottomTitles(double value, TitleMeta meta) {
      ...
      return SideTitleWidget(
        meta: meta, // This is now required
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _kv('Monthly Payment', data['calc']?['monthlyPayment']),
                    _kv('Monthly Benefit', data['calc']?['monthlyBenefit']),
                    _kv('Total Minimum Payment for Month', totalMinPayment),
                    const SizedBox(height: 8),
